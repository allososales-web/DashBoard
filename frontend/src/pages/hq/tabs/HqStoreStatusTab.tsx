          ...
          return (
            <div key={item.storeId ?? item.series ?? i}>
              ...
              <div style={{ height: 4, borderRadius: 2, background: 'var(--bg)', boxShadow: 'inset 1px 1px 3px rgba(150,158,145,0.35), inset -1px -1px 2px rgba(255,255,255,0.75)', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: ${barPct}%, borderRadius: 2, background: linear-gradient(90deg, 88, ), transition: 'width 0.6s' }} />
              </div>
            </div>
          );
        })}
          ...
          return (
            <div key={s.storeId}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 12, color: 'var(--accent)', fontWeight: 700 }}>{s.amount >= 10000 ? ${Math.round(s.amount / 10000).toLocaleString()}만원 : ${s.amount.toLocaleString()}원}</div>
...
export default function HqStoreStatusTab() {
  ...
  const { data: allMetrics = [], isLoading } = useQuery({ queryKey: ['hq-all-metrics', year, endMonth, dataMode], queryFn: () => api.get(/dashboard/all?year=&month=&dataMode=).then(r => r.data).catch(() => []) });
  const { data: seriesTopRaw = [] } = useQuery({ queryKey: ['series-top', year, startMonth, endMonth, dataMode], queryFn: () => api.get(/dashboard/series-top?year=&month=&endMonth=&dataMode=).then(r => r.data).catch(() => []), enabled: hasMetrics });
  const { data: seriesBreakdownRaw = [] } = useQuery({ queryKey: ['series-store-breakdown', year, startMonth, endMonth, dataMode], queryFn: () => api.get(/dashboard/series-store-breakdown?year=&month=&endMonth=&dataMode=).then(r => r.data).catch(() => []), enabled: hasMetrics });
  ...
  if (stores.length >= 2) {
    ...
    if (top) insights.push(${modeLabel} 1위 이 전체의 %를 차지합니다.);
    if (bottom && bottom.storeName !== top?.storeName) insights.push(${bottom.storeName ?? '하위 매장'}의 이 가장 낮습니다.);
    const quoteTop = sortedByQuote[0]; const amountRank = sortedByAmount.findIndex((s: any) => s.storeId === quoteTop?.storeId) + 1;
    if (quoteTop && amountRank > 2) insights.push(${quoteTop.storeName ?? '매장'}은 견적 1위이나  위입니다.);
  }
        ...
        <>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14 }}>
              <ExpandableRankCard title={${modeLabel} TOP} items={sortedByAmount} getValue={s => getAmt(s)} format={v => v >= 10000 ? ${Math.round(v / 10000).toLocaleString()}만원 : ${v.toLocaleString()}원} sub={s => ${getCnt(s)}건} />
              <ExpandableRankCard title={${modeLabel}건수 TOP} items={sortedByCount} getValue={s => getCnt(s)} format={v => ${v}건} sub={s => getAmt(s) >= 10000 ? ${Math.round(getAmt(s) / 10000).toLocaleString()}만원 : ${getAmt(s).toLocaleString()}원} />
          ...
          <div>
            ...
            {!hasMetrics ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 14 }}>
                <ExpandableRankCard title={품목별  TOP} items={sortedBySeriesAmount} getValue={d => d.amount} format={v => v >= 10000 ? ${Math.round(v / 10000).toLocaleString()}만원 : ${v.toLocaleString()}원} sub={d => ${d.count}건} />
                <ExpandableRankCard title={품목별 건수 TOP} items={sortedBySeriesCount} getValue={d => d.count} format={v => ${v}건} sub={d => d.amount >= 10000 ? ${Math.round(d.amount / 10000).toLocaleString()}만원 : ${d.amount.toLocaleString()}원} />
                <ExpandableRankCard title="품목별 평균단가 TOP" items={sortedBySeriesAvg} getValue={d => d.avgPrice} format={v => v >= 10000 ? ${Math.round(v / 10000).toLocaleString()}만원 : ${v.toLocaleString()}원} sub={d => ${d.count}건} />
              </div>
            )}
          </div>
              ...
              <table>
                ...
                <tbody>
                  {sortedByAmount.map((s: any) => {
                    ...
                    return (<tr key={s.storeId}><td style={{ fontWeight: 500, whiteSpace: 'nowrap' }}>{s.storeName ?? '-'}</td><td style={{ textAlign: 'right' }}>{Math.round(getAmt(s) / 10000).toLocaleString()}만원</td><td style={{ textAlign: 'right' }}>{getCnt(s)}건</td><td style={{ textAlign: 'right' }}>{s.quoteCount ?? 0}건</td><td style={{ textAlign: 'right', color: Number(conv) >= 50 ? 'var(--success)' : 'var(--warning)' }}>{conv}%</td><td style={{ textAlign: 'right', color: 'var(--accent)' }}>{pct}%</td></tr>);
                  })}
